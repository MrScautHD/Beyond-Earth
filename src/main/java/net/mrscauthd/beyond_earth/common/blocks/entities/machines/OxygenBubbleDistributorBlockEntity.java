...
import net.mrscauthd.beyond_earth.common.machines.power.PowerSystemEnergyCommon;
...
public class OxygenBubbleDistributorBlockEntity extends OxygenMakingBlockEntity {
    ...
    protected boolean canActivated() {
        if (this.getOutputTank().getOxygen() >= this.getOxygenUsing(this.getRange())) {
            return true;
        }
    ...
    private void spawnOxygenBubble(double range) {
        ...
        for (Mob entity : entities) {
            entity.addEffect(new MobEffectInstance(EffectRegistry.OXYGEN_EFFECT.get(), 2 * 24, 0, false, false));
        }
    ...
    public int getRange() {
        return Math.max(this.getPersistentData().getInt(KEY_RANGE), Config.OXYGEN_BUBBLE_DISTRIBUTOR_RANGE_MIN.get());
    }

    public void setRange(int range) {
        range = Math.min(Math.max(range, Config.OXYGEN_BUBBLE_DISTRIBUTOR_RANGE_MIN.get()),
                Config.OXYGEN_BUBBLE_DISTRIBUTOR_RANGE_MAX.get());

        if (this.getRange() != range) {
            this.getPersistentData().putInt(KEY_RANGE, range);
            this.setChanged();
        }
    }

    public boolean isWorkingAreaVisible() {
        return this.getPersistentData().getBoolean(KEY_WORKINGAREA_VISIBLE);
    }

    public void setWorkingAreaVisible(boolean visible) {
        if (this.isWorkingAreaVisible() != visible) {
            this.getPersistentData().putBoolean(KEY_WORKINGAREA_VISIBLE, visible);
            this.setChanged();
        }
    }
    ...
    public static class ChangeRangeMessage {
        ...
        public static void handle(ChangeRangeMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
            ...
            context.enqueueWork(() -> {
                OxygenBubbleDistributorBlockEntity blockEntity = (OxygenBubbleDistributorBlockEntity) context
                        .getSender().level.getBlockEntity(message.blockPos);
    ...
    public static class ChangeWorkingAreaVisibleMessage {
        ...
        public static void handle(ChangeWorkingAreaVisibleMessage message,
