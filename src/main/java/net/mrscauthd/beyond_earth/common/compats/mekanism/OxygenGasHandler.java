...
public class OxygenGasHandler implements IGasHandler {
    ...
    public GasStack getChemicalInTank(int var1) {
        return this.createGasStack(this.getOxygenStorage().getOxygen());
    }

    @Override
    public long getTankCapacity(int var1) {
        return this.getOxygenStorage().getMaxCapacity();
    }
    ...
    public void setChemicalInTank(int var1, GasStack var2) {
        if (this.isCanInsert() && this.isValid(var1, var2)) {
            this.getOxygenStorage().setOxygen((int) var2.getAmount());
        }
    }
