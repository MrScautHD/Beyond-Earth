...
public class VenusBullet extends Structure {
    ...
    public static final Codec<VenusBullet> CODEC = RecordCodecBuilder.<VenusBullet>mapCodec(instance ->
            instance.group(VenusBullet.settingsCodec(instance),
            ...
            ).apply(instance, VenusBullet::new)).codec();

    public VenusBullet(Structure.StructureSettings config, Holder<StructureTemplatePool> startPool, Optional<ResourceLocation> startJigsawName, int size, HeightProvider startHeight, Optional<Heightmap.Types> projectStartToHeightmap, int maxDistanceFromCenter) {
        super(config);
    ...
    public StructureType<?> type() {
        return StructuresRegistry.VENUS_BULLET.get();
    }
}